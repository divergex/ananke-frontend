...
export function graphToXml(graph: SerializedGraph): string {
    const nodeLines = graph.nodes.map((n) => {
        const paramLines = n.params.map(
            (p) => `      <property name="${esc(p.name)}" value="${esc(p.value)}"/>`,
        );
        const inner = paramLines.length > 0 ? `\n${paramLines.join('\n')}\n    ` : '';
        return `    <node id="${esc(n.id)}" type="${esc(n.type)}" label="${esc(n.label)}" x="${n.x.toFixed(
            1,
        )}" y="${n.y.toFixed(1)}">${inner}</node>`;
    });

    const edgeLines = graph.edges.map(
        (e) =>
            `    <edge src_node="${esc(e.src_node)}" src_port="${esc(e.src_port)}" dst_node="${esc(
                e.dst_node,
            )}" dst_port="${esc(e.dst_port)}"/>`,
    );
...
export function xmlToGraph(xml: string): SerializedGraph {
    const obj = parser.parse(xml)?.graph;
    if (!obj) throw new Error('Invalid XML: no <graph> root');

    const nodes: SerializedNode[] = [];
    const edges: SerializedEdge[] = [];

    const nodeArr = Array.isArray(obj.node) ? obj.node : obj.node ? [obj.node] : [];
    for (const n of nodeArr) {
        const paramsArr = Array.isArray(n.param) ? n.param : n.param ? [n.param] : [];
        const params: SerializedParam[] = paramsArr.map((p: any) => ({
            name: p.name ?? '',
            value: p.value ?? '',
        }));
        nodes.push({
    ...
    const edgeArr = Array.isArray(obj.edge) ? obj.edge : obj.edge ? [obj.edge] : [];
...
export type ValidationIssue = { level: 'error' | 'warn'; message: string; nodeId?: string };
...
export function validateGraph(graph: SerializedGraph): ValidationIssue[] {
    ...
    for (const n of graph.nodes) {
        ...
        for (const port of def.inputs) {
            if (port.required !== false && !connectedDst.has(`${n.id}:${port.name}`)) {
                issues.push({
                    level: 'error',
